...
class AddFavoriteUseCase:
    ...
    async def execute(self, client_id: int, product_id: int) -> int:
        logger.info(
            f"Adding favorite item for client_id: {client_id}, product_id: {product_id}"
        )
        if not await self._check_product_exists(product_id):
        ...
        if await self._check_favorite_exists(client_id, product_id):
            logger.error(
                f"Favorite item with product_id: {product_id} already exists "
                f"for client_id: {client_id}"
            )
            raise FavoriteProductAlreadyExistsError(
                title="Favorite already exists",
                detail=f"Favorite item with product_id: {product_id} already exists "
                f"for client_id: {client_id}",
                status_code=400,
            )

        result = await self.favorite_repository.add_favorite(client_id, product_id)
        logger.info(
            f"Favorite item added for client_id: {client_id}, item_id: {result}"
        )
        return result
